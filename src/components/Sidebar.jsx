    ...
    <>
      ...
      >
        ...
        <div className="flex gap-x-4 items-center">
          <img src={logo} className={`cursor-pointer duration-500 ${isSidebarOpen && "rotate-[360deg]"}`} />
        </div>
            ...
            <div key={index}>
              ...
              >
                <Link to={Menu.path || "#"} className="flex items-center gap-x-4 w-full">
                  ...
                  <span className={`${!isSidebarOpen && "hidden"} origin-left duration-200`}>{Menu.title}</span>
                ...
                <ul className="ml-6 transition-all duration-300">
                  {Menu.submenus.map((submenu, subIndex) => (
                    <li key={subIndex} className="mt-2">
                      <Link to={submenu.path} className="text-gray-400 hover:text-white text-sm">
                        - {submenu.title}
                      </Link>
                    </li>
                  ))}
