...
const ReactTableBUAllocation = ({ columns, rows, setRows, loadingTable, setLoadingTable, handleCellEditCommit, isSubmitted }) => {
    ...
    const nonEditableColumns = ["EmpCode", "Reporting Head", "Total", "EmpName", "Business Unit", "MaxAllocation", "CC_Code", "CCName"];
    ...
    const onCellEditComplete = (event) => {
        ...
        if (newTotal > rowData.MaxAllocation) {
            // if (newTotal > 100) {
            toastRef.current.show({
                severity: "error",
                summary: "Limit Exceeded",
                detail: `Total cannot be more than ${rowData.MaxAllocation}`,
                life: 3000,
            });
    ...
    const normalColumns = columns.filter(col => col.field !== "Total");
    const totalColumn = columns.find(col => col.field === "Total");
                                ...
                                body={(rowData) =>
                                    col.field === "Total" ? (
                                        ...
                                        <div
                                            style={{
                                                color:
                                                    Number(rowData[col.field]) >= 1 &&
                                                        Number(rowData[col.field]) <= 100
                                                        ? "red"
                                                        : undefined,
                    ...
                    <Column
                        ...
                        body={(rowData) => (
                            <div
                                style={{
                                    backgroundColor:
                                        rowData.Total !== rowData.MaxAllocation
                                            ? "rgba(255, 99, 71, 0.3)"
                                            : "rgba(144, 238, 144, 0.5)",
                                    color: rowData.Total !== rowData.MaxAllocation ? "red" : "green",
                                    fontWeight: "bold",
                                    padding: "5px",
                                    textAlign: "center",
                                }}
                            >
                                {/* {rowData.Total} */}
                                {Math.round(Number(rowData.Total))}
                            </div>
                        )}
                        bodyStyle={{
                            textAlign: "center",
                            // backgroundColor: "#f1f5f9",
                            boxShadow: "-4px 0 6px rgba(0,0,0,0.08)", // 👈 nice frozen effect
                        }}
                    />
                )}
...
export default ReactTableBUAllocation;