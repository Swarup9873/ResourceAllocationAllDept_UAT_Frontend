...
//import DataGridProjectAllocation from "../components/datagrids/DataGridProjectAllocation"
import DataGridProjectAllocation from "../components/datagrids/ReactTable"
//import DataGridProjectAllocation from "../components/datagrids/ReactTableCopy"
...
const AllocateProjects = () => {
    ...
    data.forEach(entry => {
      ...
      Object.entries(projects).forEach(([project, allocation]) => {
        ...
        if (parts.length === 2) {
          ...
          const projectId = parts[1];    // Number after ##
          ...
          if (allocation !== null && allocation !== "0" && allocation !== 0) {
            result.push({
              ...
              empCode: EmpCode,
              empName: EmpName,
              projectId: projectId,  // Add extracted ProjectId
              projectName: projectName,
            });
          }

          // result.push({
          //   allocationRate: allocation ?? 0, // Default allocation to 0 if null
          //   empCode: EmpCode,
          //   empName: EmpName,
          //   projectId: projectId,  // Add extracted ProjectId
          //   projectName: projectName,
  ...
  const handleSave = async (e) => {
    ...
    try {
      const response = await axios.post(`${base_URL}/api/ProjectAllocation/Allocate`,
        {
  ...
  const handleClone = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(`${base_URL}/api/ProjectAllocation/Details?EmpCode=${empCode}&Month=${month}&Year=${year}`
        ,
        {
          headers: {
            'Authorization': `Bearer ${Token}`
      ...
      const nonEditableColumns = ["EmpCode", "TOTAL", "EmpName", "Reporting Head"];
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(`${base_URL}/api/ProjectAllocation/Details?EmpCode=${empCode}&Month=${formData.month}&Year=${formData.year}`
        ,
        {
          headers: {
            'Authorization': `Bearer ${Token}`
  ...
  const validateRows = () => {
    const hasInvalidRows = rows.some(row => {
      return row.Total !== 100; // Check if Total is NOT equal to 100
    });
  ...
  const fetchProjectAllocation = async () => {
    ...
    try {
      setLoadingTable(true);
      // const response = await axios.get(`${base_URL}/api/ProjectAllocation/Details?Month=${month}&Year=${curr_year}`);
      const response = await axios.get(`${base_URL}/api/ProjectAllocation/Details?EmpCode=${empCode}&Month=${currentMonth}&Year=${curr_year}`
        ,
        {
          headers: {
            'Authorization': `Bearer ${Token}`
