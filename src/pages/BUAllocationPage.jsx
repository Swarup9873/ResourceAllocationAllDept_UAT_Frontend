...
import ReactTableBUAllocation from "../components/datagrids/ReactTableBUAllocation";
...
const BUAllocationPage = () => {
  ...
  function transformData(data) {
    ...
    data.forEach(entry => {
      const { EmpCode, CC_Code, ...projects } = entry;
      ...
      Object.entries(projects).forEach(([project, allocation]) => {
        ...
        if (parts.length === 2) {
          const DeptName = parts[0];  // Name before ##
          const BU_Code = parts[1];    // Number after ##
  ...
  const handleSave = async (e) => {
    ...
    try {
      const response = await axios.post(`${base_URL}/api/BUAllocation/Allocate/BU`,
        {
  ...
  const handleClone = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(`${base_URL}/api/BUAllocation/Details/BU?EmpCode=${formData.departmentId}&Month=${month}&Year=${year}`
        ,
        {
          headers: {
            'Authorization': `Bearer ${Token}`
      ...
      const dynamicColumns = Object.keys(firstItem)
        .filter((key) => !key.includes("CC_Code"))
        .map((key) => ({
          field: key,
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(`${base_URL}/api/BUAllocation/Details/BU?EmpCode=${selectedEmployee.ECN}&Month=${formData.month}&Year=${formData.year}`
        ,
        {
          headers: {
            'Authorization': `Bearer ${Token}`
      ...
      if (data.length === 0) {
        toast.warn("No BU allocation data found for the selected month and year.");
        return;
      }
      ...
      console.error("Error fetching BU allocation:", error);
      toast.error("Error fetching BU allocation :" + error.message);
    }
    finally {
      setLoadingTable(false);
  ...
  const handleDownloadExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "BUAllocation");
      ...
      saveAs(data, `BU_Allocation_${user}_${dmonth}.xlsx`);
    } catch (error) {
  ...
  const fetchProjectAllocation = async () => {
    ...
    try {
      setLoadingTable(true);
      const response = await axios.get(`${base_URL}/api/BUAllocation/Details/BU?EmpCode=${selectedEmployee.ECN}&Month=${currentMonth}&Year=${curr_year}`
        ,
        {
          headers: {
            'Authorization': `Bearer ${Token}`
      ...
      if (data.length > 0) {
        ...
        const nonEditableColumns = ["EmpCode", "TOTAL", "EmpName", "Reporting Head", "MaxAllocation", "CC_Code", "CCName"];
      ...
      console.error('Error fetching BU Allocation:', error);
      toast.error("Something Went wrong: ", error.returnMessage);
    }
    finally {
      setLoadingTable(false);
    }
    ...
    <div className="relative">
      ...
      <div className="p-2 w-[90%] flex justify-end mx-auto mt-5">
        <form onSubmit={handleSearch} className="flex items-center space-x-2">
          {/* Department Dropdown */}
          <EmployeeDropdown selectedEmployee={selectedEmployee} setSelectedEmployee={setSelectedEmployee} reload={true} />
        ...
        <div className="border border-gray-300 p-1 bg-white shadow-md rounded-lg">
          <div className="flex justify-between bg-dark-purple">
            <Typography variant="h6" className="text-white font-medium p-1">
              BU Profit Center Allocation ({dmonth})
            </Typography>
...
export default BUAllocationPage;